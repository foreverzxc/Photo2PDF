...
public:
    explicit ExportPDFWorker(const QStringList &fileNames,
                             const QStringList &pages,
                             const QList<Transformers> &transformersList,
                             const ExportPDFConfig &config,
    ...
    void ExportPDF() {
        ...
        for (int i = 0; i < totalFiles; ++i) {
            ...
            if(pages[i] == "") {
                ...
                int page = pages[i].toInt();  // getAllPageNumbers returns 0-based page numbers
                QPdfDocument document;
                document.load(imagePath);

                // Check if document loaded successfully
                if (document.status() != QPdfDocument::Status::Ready) {
                    emit ErrorOccurredSignal("Failed to load PDF: " + imagePath);
                    continue;
                }

                if (page < 0 || page >= document.pageCount()) {
                    emit ErrorOccurredSignal("Invalid page number: " + QString::number(page + 1));
                    continue;
                }
